...
export default (() => {
  let process = async (ctx, next) => {
    ...
    switch (command) {
      case "login":
        let UserRs = await new BaseDao("users").retrieve({
          username: ctx.request.body.username,
          password: ctx.request.body.password,
        });
        if (UserRs.status === G.STCODES.SUCCESS) {
          let user = UserRs.data[0];
          let ars = await new MenuDao("").create({ id: user.id });
          if (ars.status === G.STCODES.SUCCESS) {
            ...
            ctx.body = G.jsResponse(G.STCODES.SUCCESS, "login success.", {
              token,
              u_id: user.id,
            ...
            ctx.body = G.jsResponse(
              G.STCODES.QUERYEMPTY,
              "The AuthDao is missing."
            );
          }
        } else {
      ...
      case "upload":
        try {
          ...
          if (files) {
            ...
            return (ctx.body = G.jsResponse(
              G.STCODES.UPLOADERR,
              "file upload fail. no file found."
            ));
          }
        } catch (err) {
          return (ctx.body = G.jsResponse(
            G.STCODES.EXCEPTIONERR,
            "file upload fail. error: " + err
          ));
        }
      case "fetch":
...
const uploadFilePublic = function (files) {
  ...
  const fileFunc = function (file) {
    ...
    file.newName = `${moment().format("YYYYMMDD")}_${G.tools.uuid()}.${
  ...
  const returnFunc = function () {
    ...
    return G.jsResponse(G.STCODES.SUCCESS, "file upload success", {
      url,
      oldName,
    });
  };
